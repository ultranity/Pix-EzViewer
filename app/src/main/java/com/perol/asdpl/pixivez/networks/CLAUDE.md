...
- **Cloudflare 共享 anycast**(公共 DNS/DoH 返回,`104.x`/`172.64.x`,AS13335):
  无 SNI 时选不出证书 → TLS 握手失败;须带明文/ECH SNI。

故 DNS 与 SNI 两维度**有耦合**(空 SNI 须配源站):

- **DNS 维度** —— 连哪类入口:DIRECT(Pixiv 源站)/ DoH(anycast)/ 系统。
- **SNI 维度** —— ClientHello 呈现:替换(pixiv.me)/ 空 / 明文(可被 GFW RST)/ ECH。

墙内可用组合 = **DIRECT(直连源站) + 替换 SNI=pixiv.me**(空 SNI 亦可,但部分入口
默认证书不覆盖三段域名会 421);无墙/代理 = DoH + 明文;墙内加密 = anycast + ECH(Phase 2)。

## 文件职责

| 文件 | 职责 |
|------|------|
| `NetworkMode.kt` | **API 连接两轴模型**:`DnsMode`(direct/doh/system)× `SniMode`(replace/empty/plain/ech)+ `DohConfig`(默认 cloudflare-dns.com + bootstrap)+ `SniReplaceConfig`(替换 SNI 主机,默认 pixiv.me)+ `DohApiDns`(DoH→anycast)+ `PixivDirectDns`(源站直连 IP)+ `EchTransport`(Phase 2 占位)+ `applyApiNetwork()`。 |
...
| `Pkce.kt` | OAuth PKCE。 |
| `RubySSLSocketFactory.kt` / `RubyX509TrustManager.kt` | 空 SNI 实现:用 `InetAddress` 重载建 SSLSocket(不发 SNI)+ 信任全部证书。供 API 的 `SniMode.EMPTY` 与图片路径复用。 |
...
- `DOH`:经 `DohConfig`(默认 `cloudflare-dns.com` + bootstrap IP)→ Cloudflare anycast;配明文/ECH。
- `SYSTEM`:系统 DNS。无墙 / 走代理 / VPN。

**SniMode**(pref `apiSniMode`,默认 `replace`)
- `REPLACE`:替换 SNI 为 `SniReplaceConfig`(默认 `pixiv.me`,pref `apiSniReplaceHost`)+ 信任全部([ReplaceSniSocketFactory])。pixiv.me 不被 GFW 封,且其多-SAN 证书(含 oauth.secure 等)能授权目标 Host → 不致 421。**须配 `DIRECT`**。
- `EMPTY`:空 SNI + 信任全部([RubySSLSocketFactory])。须配 `DIRECT`;部分入口默认证书不覆盖三段域名(如 oauth.secure)会 421。
- `PLAIN`:明文真实 SNI + 证书校验。配 DoH/anycast;无墙 / 代理可用;墙内会被 SNI RST。
- `ECH`:加密 SNI;Phase 2,就绪前**不在 UI 暴露**,回退 `REPLACE`。

默认 **DIRECT + REPLACE(pixiv.me)**(墙内可用)。`DohApiDns` 经 DoH 动态解析 anycast IP,
硬编码仅兜底;`PixivDirectDns` 硬编码源站 IP(不在公共 DNS,会随 Pixiv 轮换,失效需更新)。

设置入口:`SettingsFragment.showAPIConfigDialog()` 的「DNS 解析」「SNI 模式」下拉 +
「DoH 服务商」输入框。旧的 `dnsProxy` 开关现仅影响图片路径。
